...
* Process **input** information to generate desired **output** information
...
* Opcodes & operands are **binary code** (binary patterns)
...
Example instructions (Intel 8085):
...
**Storage device** (integrated circuit) holding information temporarily or permanently:

* Stores information in **binary representation** (binary data)
* Processor **register** ➜ Internal integrated (small) fast memory (may be read or write only)
...
* Temporary **primary storage** (volatile):
  - Processor **cache** is an intermediate stage between ultra-fast processor registers and much slower & bigger main memory 
  - Volatile **main memory** (Random Access Memory (RAM)) operating at high speed compared to secondary storage
* Permanent **secondary storage** (persistent):
